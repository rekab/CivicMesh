...
Two host processes hold open file descriptors on the same `/dev/ttyUSB0` simultaneously. Linux does not exclusive-lock USB-serial nodes; pyserial opens without `TIOCEXCL`, so both `open()` calls succeed at the kernel level. Inbound bytes from the radio are split arbitrarily between the two readers (each sees fragments); outbound writes from the two processes interleave on the wire. The radio sees corrupt frames; the host-side parser hunts for `0x3E` start bytes in mid-frame garbage (`serial_cx.py:79`).

**Mitigation (CIV-80, present):** `civicmesh-mesh` takes `fcntl.flock(LOCK_EX | LOCK_NB)` on `/run/lock/civicmesh-mesh.lock` at startup via `process_lock.acquire_mesh_bot_lock()`. The file is pre-created by `/etc/tmpfiles.d/civicmesh.conf` (shipped via `apply`) at mode `0664 root:dialout` so both the installed service user and any dev user in `dialout` can flock it. A second `civicmesh-mesh` invocation exits immediately with a `RuntimeError` naming the lock path. So the **common operator-error case** — a developer running `uv run civicmesh-mesh` from `~/code/CivicMesh/` without first stopping the installed systemd service — now fails loudly, not silently.

The flock is **cooperative**: only processes that opt into the lock are blocked. C3 can still occur if:

- A non-CivicMesh tool grabs the device — `picocom /dev/ttyUSB0`, `screen /dev/ttyUSB0`, `cat /dev/ttyUSB0`, `miniterm.py`, etc.
- A diagnostic script in `diagnostics/radio/*.py` opens the device directly via `serial.Serial(...)` without the lock (they assume `civicmesh-mesh` is stopped per their preconditions; if that assumption is violated, C3 returns).
- An older `civicmesh-mesh` binary without the CIV-80 lock is run alongside a current one.

When C3 does occur, the symptoms below apply:
...
- **Recovery:** stop the duplicate process. Concrete command: `sudo systemctl stop civicmesh-mesh` (or `kill <PID>` for an offending dev process or terminal tool), then restart whichever side should remain. RTS pulse, ESP32 reset, and VBUS cycle do **not** help — they do not close the host file descriptors.
- **Detection:** **`lsof /dev/ttyUSB0`** and **`fuser /dev/ttyUSB0`** will both list multiple PIDs. That is the unambiguous signal. `lsof /run/lock/civicmesh-mesh.lock` additionally identifies the holding `civicmesh-mesh` PID directly. Without running these commands, the symptoms look identical to C1/C2 — an agent debugging "framing errors" should rule C3 out before chasing the radio. See also `docs/civicmesh-tool.md` (section "Running dev alongside prod") for the operator-side recipe.
- **Likelihood:** **L** — operator-induced, not hardware-driven. Pre-CIV-80 the dominant trigger was the dev-vs-prod `civicmesh-mesh` collision; that case is now blocked at startup. Remaining triggers are non-CivicMesh tools and unguarded diagnostic scripts.
